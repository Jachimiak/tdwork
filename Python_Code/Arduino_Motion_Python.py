#
//  Arduino_Motion_Python.py
//  
//
//  Created by Terry Jachimiak on 3/10/26.
//

# me - this DAT.
#
# dat - the changed DAT
# rows - a list of row indices
# cols - a list of column indices
# cells - the list of cells that have changed content
# prev - the list of previous string contents of the changed cells
#
# Make sure the corresponding toggle is enabled in the DAT Execute DAT.
#
# If rows or columns are deleted, sizeChange will be called instead of row/col/cellChange.

# me - this DAT
# dat - the changed DAT
import time

def ignoreForSeconds(seconds):
    """
    Call this function to ignore messages for a specified duration
    """
    me.store('ignoreUntil', time.time() + seconds)
    print(f"Ignoring serial messages for {seconds} seconds")
    
def stopIgnoring():
    """
    Call this to immediately stop ignoring messages
    """
    me.store('ignoreUntil', 0)
    print("No longer ignoring serial messages")
    
def onTableChange(dat):
    # Fetch ignoreUntil time (default 0)
    ignoreUntil = me.fetch('ignoreUntil', 0)
    print(ignoreUntil)
    currentTime = time.time()
    print(currentTime)
    
    # If still in ignore period, exit early
    if currentTime < ignoreUntil:
        return
        
    # Process only if there’s at least one data row
    if dat.numRows > 1:
        message = dat[1, 0].val.strip()
        
        print(f"Received: {message}")  # Debug output
        
        if message == "On":
            print("Triggering Pattern CHOP!")
            op('pattern1').par.randomize.pulse()
            if op('switch1').par.index == 0:
                op('kentvideo').par.cue.pulse()
                op('forty').par.start.pulse()
                op('kentaudio').par.cue.pulse()
                ignoreForSeconds(60)
                return
            elif op('switch1').par.index == 1:
                op('firevideo').par.cue.pulse()
                op('fireaudio').par.cue.pulse()
            elif op('switch1').par.index == 2:
                op('whispersvideo').par.cue.pulse()
                op('twenty').par.start.pulse()
                op('whispersaudio').par.cue.pulse()
            elif op('switch1').par.index == 3:
                op('underwatervideo').par.cue.pulse()
                op('twenty').par.start.pulse()
                op('underwateraudio').par.cue.pulse()
            elif op('switch1').par.index == 4:
                op('glitchvideo').par.cue.pulse()
                op('glitchaudio').par.cue.pulse()
            elif op('switch1').par.index == 5:
                op('creepychild').par.cue.pulse()
                op('creepychildaudio').par.cue.pulse()
                
               
                
           
            # Ignore further messages for 40 seconds
            # This counts for 20 seconds of the clip and 20 more seconds as people walk through
            ignoreForSeconds(40)
        
def onRowChange(dat, rows):
    return

def onColChange(dat, cols):
    return

def onCellChange(dat, cells, prev):
    return

def onSizeChange(dat):
    return
